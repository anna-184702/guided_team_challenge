...
USER_NAME   = "av555@cam.ac.uk"  # Replace with API username (string)
API_KEY     = "f9bb2b04-79fe-459f-96fe-f4875fea6bc4"  # Replace with API key      (string)
...
def fields(payload=None):
    try:
        if payload != None:
            if payload == {}:
                url      = parse.urljoin(BPD_URL, "/api/v2/introspection/fields")
                response = requests.get(url)
                return response.json()
            else:
                url      = parse.urljoin(BPD_URL, "/api/v2/introspection/fields?{}={}".format(list(payload.keys())[0], payload[list(payload.keys())[0]]))
                response = requests.get(url)
                return response.json()
...
def group_by(payload={}):
    if payload == {}:
        url      = parse.urljoin(BPD_URL, "/api/v2/introspection/group_by")
        response = requests.get(url)
        return response.json()
    else:
        url      = parse.urljoin(BPD_URL, "/api/v2/introspection/group_by?{}={}".format(list(payload.keys())[0], payload[list(payload.keys())[0]]))
        response = requests.get(url)
        return response.json()

def count(filters={}, recalculate=True):
    url      = parse.urljoin(BPD_URL, "/api/v2/analyze/count")
...
def histogram(filters={}, group_by="floor_area", recalculate=True, resolution="high"):
    url      = parse.urljoin(BPD_URL, "/api/v2/analyze/histogram")
    payload  = {"filters": filters,
                "group_by": group_by,
                "recalculate": recalculate,
...
def scatterplot(filters={}, xaxis="", yaxis="", addfields=[], limit=1000, seed=""):
    url     = parse.urljoin(BPD_URL, "/api/v2/analyze/scatterplot")
    payload = {"filters": filters,
...
def table(filters={}, group_by="floor_area", analyze_by="source_eui", recalculate=True):
    url     = parse.urljoin(BPD_URL, "/api/v2/analyze/table")
    payload = {"filters": filters,
               "recalculate": recalculate}
...
def comparepeergroup(from_filters, to_filters, analyze_by, base={}, method="actuarial", seed=[], recalculate=True):
    url = parse.urljoin(BPD_URL, "/api/v2/analyze/compare/peer-group")
